...
class ZJRegisterViewController: ZJMainVc {
    ...
    var passwordOrRegister: Int? {
        didSet{
            
            if passwordOrRegister == 0 {
                self.title = "注册"
            }else if passwordOrRegister == 1 {
                self.title = "找回密码"
            }
        }
    }
...
extension ZJRegisterViewController {
    ...
    @objc private func didClickSubmitButton(button: UIButton) {
        ...
        let vc = DDRegisterAgreementViewController()
        
        vc.verifty = registerView.txtInputVerifty.text
        vc.password = registerView.txtInputPassword.text
        vc.register = registerView
        vc.delegateR = self.delegate
        ...
        dispatch_source_set_event_handler(loginTimer, {
            ...
            if loginTimeOut <= 0 {
                ...
                dispatch_sync(dispatch_get_main_queue(), {
                    ...
                    /** 设置界面的按钮显示*/
                    self.registerView.btnSendVerify.setTitle(NSString.localizedStringWithFormat("%@", strTime) as String, forState: UIControlState.Normal)
