...
class DDTestModel: NSObject {
    ...
    class func loadHomeData(finished: ((result: [DDTestModel]?) -> ())) {
        
        var models = [DDTestModel]()
        
        
        CHNetworkTools.sharedNetworkTool.get(POLITICALBOOKCASE_URL+String(num2), parameters: nil) { (success, result, error) in
//            print(result!["list"])
...
//            print(data)
            ...
            for dict  in data! {
                models.append(DDTestModel(dict: dict))
            }
