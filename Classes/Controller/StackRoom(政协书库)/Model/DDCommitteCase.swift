...
class DDCommitteCase: NSObject {
    ...
    class func loadHomeData(finished: ((result: [DDCommitteCase]?) -> ())) {
        
        var models = [DDCommitteCase]()
        
        
        CHNetworkTools.sharedNetworkTool.get(POLITICALBOOKCASE_URL+String(num6), parameters: nil) { (success, result, error) in
            //            print(result!["list"])
            ...
            for dict  in data! {
                models.append(DDCommitteCase(dict: dict))
            }
