...
class DDPoliticsYearbook: NSObject {
    ...
    class func loadHomeData(finished: ((result: [DDPoliticsYearbook]?) -> ())) {
        
        var models = [DDPoliticsYearbook]()
        
        
        CHNetworkTools.sharedNetworkTool.get(POLITICALBOOKCASE_URL+String(num3), parameters: nil) { (success, result, error) in
            //            print(result!["list"])
            ...
            for dict  in data! {
                models.append(DDPoliticsYearbook(dict: dict))
            }
