...
class DDPolifitsResearch: NSObject {
    ...
    class func loadHomeData(finished: ((result: [DDPolifitsResearch]?) -> ())) {
        
        var models = [DDPolifitsResearch]()
        
        
        CHNetworkTools.sharedNetworkTool.get(POLITICALBOOKCASE_URL+String(num4), parameters: nil) { (success, result, error) in
            //            print(result!["list"])
            ...
            for dict  in data! {
                models.append(DDPolifitsResearch(dict: dict))
            }
