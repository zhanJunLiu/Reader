...
//        headView.nameLabel.text = nikeNameTableViewCell?.UserInfoName.text
        ...
        //        let data = ZJAccountModel.shareAccount()?.avatar?.dataUsingEncoding(NSUTF8StringEncoding)
        
        CHProgressHUD.showWithStatus("更新资料")
        ZJAccountModel.updateUserInfo(ZJAccountModel.shareAccount()?.nickname ?? "", sex: (sexTableViewCell?.UserInfoSex.text)!, age: Int((ageTableViewCell?.UserInfoBirthName.text)!)!) { (success) in
            
            CHProgressHUD.showSuccessWithStatus("更新成功")
            if success {
                ZJAccountModel.getSelfUserInfo({ (success) in
                    if success {
                        
                        self.delegate?.userInfoName!((ZJAccountModel.shareAccount()?.nickname)!)
