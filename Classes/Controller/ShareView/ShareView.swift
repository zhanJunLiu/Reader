...
class ShareView: UIView , UIScrollViewDelegate {
    ...
    override init(frame: CGRect) {
        ...
        let sharePaltformArray = [
            [ "share_wechat", "share_friend", "share_weibo", "share_qq", "share_zone"],
            [ "微信", "朋友圈", "微博", "QQ", "QQ空间"]
        ]
        
        self.scrollView = UIScrollView.init(frame: CGRectMake(0, 0, CGRectGetWidth(frame), 165))
...
//        self.contentView.addSubview(self.pageControl)
    ...
    func shareButtonClicked(sender:UIButton) {
        ...
        if sender.tag == 100 {
            ...
            /** 分享到微信 */
            sharedThiard(.TypeWechat, shareParames: shareParames)
            ...
            if self.shareTitle != nil && self.shareTitle.characters.count > 0 {
                ...
                /** 分享到微信朋友圈 */
                sharedThiard(.SubTypeWechatTimeline, shareParames: shareParames)
            ...
            /** 分享到微博 */
            sharedThiard(.TypeSinaWeibo, shareParames: shareParames)
            ...
            /** 分享到QQ */
            sharedThiard(.TypeQQ, shareParames: shareParames)
    ...
    // 封装第三方分享方法
    private func sharedThiard(formType: SSDKPlatformType, shareParames: NSMutableDictionary) {
        
        //2.进行分享
        ShareSDK.share(formType, parameters: shareParames) { (state : SSDKResponseState, userData : [NSObject : AnyObject]!, contentEntity :SSDKContentEntity!, error : NSError!) -> Void in
            
            switch state{
                
            case SSDKResponseState.Success:
                NSNotificationCenter.defaultCenter().postNotificationName(kThirdLoginKey, object: self, userInfo: nil)
    ...
    func showInViewController(viewController:ZJMainVc){
